...
RasterizeGaussiansCUDA(
	...
	const torch::Tensor& opacity_residue,
	const torch::Tensor& color_residue,
	...
	const bool debug)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		opacity_residue.contiguous().data<float>(), 
		color_residue.contiguous().data<float>(),
	...
	const bool debug) 
{
  ...
  int uv_res;
  if (opacity_residue.size(0) == 0) {
	  uv_res = 4;
  } else {
	  uv_res = opacity_residue.size(1);
  }
  torch::Tensor dL_dopacity_residue = torch::zeros({P, uv_res, uv_res, 3}, means3D.options());
  torch::Tensor dL_dcolor_residue = torch::zeros({P, uv_res, uv_res, 9}, means3D.options());
  ...
  {  
	  ...
	  dL_dopacity_residue.contiguous().data<float>(),
	  dL_dcolor_residue.contiguous().data<float>(),
	  debug);
  }
  // also return dL_dview2gaussian so it could be used when view2gaussian is precomputed
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dopacity_residue, dL_dcolor_residue, dL_dview2gaussian);
}
	...
	const bool debug)
{
  ...
  return std::make_tuple(rendered, out_color, out_alpha_integrated, out_color_integrated, radii, geomBuffer, binningBuffer, imgBuffer);
}