	 ...
	 for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	 {
		 ...
		 for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		 {
			 ...
			 if (con_o.w * exp(power) < 1.0f / 255.0f)
				 continue;
			float test_T_pre = T * (1 - con_o.w * exp(power));
			if (test_T_pre < 0.0001f)
			{
			 ...
			 if(add_tex){
				...
				//  float weight_3 = scale_j.z * scale_j.x / weight_total;
				 ...
				 float weight_1 = 1.0 / 3.0f;
				 float weight_2 = 1.0 / 3.0f;
				 float weight_3 = 1.0 / 3.0f;
			 ...
			 for (int ch = 0; ch < CHANNELS; ch++)
				 // C[ch] += (features[collected_id[j] * CHANNELS + ch] + this_color_residue[ch]) * alpha * T;
	 ...
	 for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	 {
		 ...
		 for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		 {
			 ...
			 for (int k = 0; k < 5; ++k){
				...
				if (con_o.w * exp(power) < 1.0f / 255.0f)
					continue;
				float test_T_pre = corner_Ts[k] * (1 - con_o.w * exp(power));
				if (test_T_pre < 0.0001f)
				{
					done = true;
					continue;
				...
				bool add_tex = con_o.w > 0.05f && (opacity_residue != nullptr) && (color_residue != nullptr);
				 ...
				 if (k == 0){
					 // Eq. (3) from 3D Gaussian splatting paper.
					 for (int ch = 0; ch < CHANNELS; ch++)
						 C[ch] += clamp(features[collected_id[j] * CHANNELS + ch] + this_color_residue[ch], 0.0f, 100.0f) * alpha * corner_Ts[k];
				 }
