	...
	bool debug)
{
	if (DEBUG_INTEGRATE && PRINT_INTEGRATE_INFO){
		printf("in CudaRasterizer::Rasterizer::integrate\n");
		printf("PN: %d P: %d, D: %d, M: %d\n", PN, P, D, M);
	}
	const float focal_y = height / (2.0f * tan_fovy);
	const float focal_x = width / (2.0f * tan_fovx);
	...
	if (DEBUG_INTEGRATE && PRINT_INTEGRATE_INFO){
		printf("==in CudaRasterizer::Rasterizer::integrate, P: %d num_rendered: %d binning_chunk_size: %d\n", P, num_rendered, binning_chunk_size);
		// for (int i = 0; i < 100; i++){
		// 	printf("geomState.tiles_touched[%d]: %d\n", i, (int)geomState.tiles_touched[i], (int)geomState.point_offsets[i]);
		// }
	}
