...
def total_variance_loss(grid):
    # Check if grid is 3D (N, res, res) or 4D (N, res, res, 3)
    if grid.dim() == 3:
        # For shape [N, res, res], compute horizontal and vertical differences
        diff_x = grid[:, :, :-1] - grid[:, :, 1:]  # Horizontal differences
        diff_y = grid[:, :-1, :] - grid[:, 1:, :]  # Vertical differences
    elif grid.dim() == 4:
        # For shape [N, res, res, 3], compute horizontal and vertical differences
        diff_x = grid[:, :, :-1, :] - grid[:, :, 1:, :]  # Horizontal differences (across width)
        diff_y = grid[:, :-1, :, :] - grid[:, 1:, :, :]  # Vertical differences (across height)
    else:
        raise ValueError(f"Unsupported grid dimension: {grid.dim()}")

    # Square the differences
    diff_x = diff_x ** 2
    diff_y = diff_y ** 2

    # Sum up all differences and normalize by the number of elements
    total_variance = (diff_x.sum() + diff_y.sum()) / grid.numel()
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    # testing_iterations = range(1000, opt.iterations + 1, 1000)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        #     rend_mask = render_pkg['render'][7, :, :]
        #     mask_loss = (rend_mask - viewpoint_cam.gt_alpha_mask.cuda()).abs().mean()
        #     loss += mask_loss * 0.1

        if 'opacity_tex' in render_pkg and render_pkg['opacity_tex'] is not None:
            opacity_tex_norm_loss = render_pkg["opacity_tex"].abs().mean()
            color_tex_norm_loss = render_pkg["color_tex"].abs().mean()
            loss += opacity_tex_norm_loss * 0.01 + color_tex_norm_loss * 0.01

        if opt.mcmc:
            opacity_norm = gaussians.get_opacity
            scaling_norm = gaussians.get_scaling
            loss += opacity_norm.abs().mean() * args.opacity_reg + scaling_norm.abs().mean() * args.scale_reg
        ...
        # if iteration % 100 == 0 and 'opacity_tex' in render_pkg and render_pkg['opacity_tex'] is not None:
        #     opacity_tex = render_pkg["opacity_tex"]
        #     color_tex = render_pkg["color_tex"]

        #     print(f'\n [INFO] opacity tex mean norm, color tex mean norm: {opacity_tex.mean().item()}, {color_tex.mean().item()}')
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}", "Points number": f"{gaussians.get_xyz.shape[0]}", "CUDA Memory": f"{torch.cuda.max_memory_allocated() / 1024**3:.2f} GB"})
            ...
            if (iteration in saving_iterations):
                ...
                gaussians.save_weights(scene.model_path, iteration)
            ...
            if iteration < opt.iterations:
                ...
                if opt.mcmc:
                    ...
                    L = build_scaling_rotation(gaussians.get_scaling_with_3D_filter, gaussians.get_rotation)
