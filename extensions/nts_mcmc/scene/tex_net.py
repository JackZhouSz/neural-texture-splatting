...
class TexNetFinal(nn.Module):
    def __init__(self, uv_res, separete_triplane = False, decompose_type='CP', cnn_generator=False):
        super(TexNetFinal, self).__init__()
        self.uv_res = uv_res
        ...
        self.opacity_dim = 3
        self.tex_color_dim = 3 * 3

        if separete_triplane:
            ...
            self.triplane_base = Triplane(triplane_feat_dim=16, cnn_generator=cnn_generator)
            self.triplane_opacity = self.triplane_color = self.triplane_base # todo: save forward pass time, reuse triplane
        ...
        self.color_net =  nn.Sequential(
                nn.Linear(self.triplane_color.output_dim + 3 + self.view_encoder[1], self.hidden_dim),
                nn.ReLU(inplace=True),
                nn.Linear(self.hidden_dim, self.hidden_dim),
                nn.ReLU(inplace=True),
                nn.Linear(self.hidden_dim, self.tex_color_dim * self.texture_model_dim),
            )

    def forward_opacity(self, query_xyz, additional_feat):
        # query_xyz = query_xyz / 3.0 * 2 - 1.0
        # query_xyz = contract_mipnerf360(query_xyz, -10, 10)
    ...
    def forward_color(self, query_xyz, additional_feat):
        ...
        if self.decompose_type == 'CP':
            ...
            color = color_base
            # color = color_base.reshape(-1, self.uv_res, self.uv_res, 3)
