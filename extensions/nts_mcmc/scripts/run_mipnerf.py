...
DATA_DIR = './data/mipnerf-360/'

output_dir = "output/mipnerf_full_release"
dry_run = False

jobs = list(zip(scenes, factors))

def train_scene(gpu, scene, factor):
    cmd = f"OMP_NUM_THREADS=4 CUDA_VISIBLE_DEVICES={gpu} python train.py -s {DATA_DIR}/{scene} -m {output_dir}/{scene} --eval --config configs/{scene}.json --capmax_ratio 0.1"
    print(cmd)
    if not dry_run:
       os.system(cmd)
