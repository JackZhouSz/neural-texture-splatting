	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (con_o.w * exp(power) < 1.0f / 255.0f)
				continue;
			bool if_all_scale_larger_0 = (scale_j.x > 1e-4f) & (scale_j.y > 1e-4f)& (scale_j.z > 1e-4f);
			// bool add_tex = con_o.w > 0.05f && (opacity_residue != nullptr) && (color_residue != nullptr) & if_all_scale_larger_0;
			...
			if(add_tex){
				...
				double3 ray_local_scaled = {
					ray_local.x / (fabs(scale_j.x) < EPS ? copysign(EPS, scale_j.x) : scale_j.x),
					ray_local.y / (fabs(scale_j.y) < EPS ? copysign(EPS, scale_j.y) : scale_j.y),
					ray_local.z / (fabs(scale_j.z) < EPS ? copysign(EPS, scale_j.z) : scale_j.z)
				};
				
				double3 cam_pos_local_scaled = {
					cam_pos_local.x / (fabs(scale_j.x) < EPS ? copysign(EPS, scale_j.x) : scale_j.x),
					cam_pos_local.y / (fabs(scale_j.y) < EPS ? copysign(EPS, scale_j.y) : scale_j.y),
					cam_pos_local.z / (fabs(scale_j.z) < EPS ? copysign(EPS, scale_j.z) : scale_j.z)
				};
				...
				float scale_x = fabs(scale_j.x);
				float scale_y = fabs(scale_j.y);
				float scale_z = fabs(scale_j.z);
	
				float weight_total = scale_x * scale_y + scale_y * scale_z + scale_z * scale_x + 1e-6;
				weight_1 = scale_x * scale_y / weight_total;
				weight_2 = scale_y * scale_z / weight_total;
				weight_3 = scale_z * scale_x / weight_total;
			...
			float alpha_gain = 1.0f + this_opacity_residue;              // (1 + r)
			float alpha_base = con_o.w * G;
			float alpha = fminf(0.99f, fmaxf(0.0f, alpha_base * alpha_gain));
			...
			if (add_tex){
				...
				bilinear_interpolation_float_grad_full_triplane(query_uv_1, uv_res, query_opacity_residue_1, dL_dopacity_residue, global_id, 0, dL_dalpha * weight_1 * alpha_base, dL_duv);
				bilinear_interpolation_float_grad_full_triplane(query_uv_2, uv_res, query_opacity_residue_2, dL_dopacity_residue, global_id, 1, dL_dalpha * weight_2 * alpha_base, dL_duv);
				bilinear_interpolation_float_grad_full_triplane(query_uv_3, uv_res, query_opacity_residue_3, dL_dopacity_residue, global_id, 2, dL_dalpha * weight_3 * alpha_base, dL_duv);
			}

			// Helpful reusable temporary variables
			// const float dL_dG = con_o.w * dL_dalpha;
			const float dL_dG = con_o.w * dL_dalpha * alpha_gain;
			...
			atomicAdd(&(dL_dopacity[global_id]), G * dL_dalpha * alpha_gain);
		}
	}
}
