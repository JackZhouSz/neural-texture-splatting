	...
	int* __restrict__ is_used)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float test_T_pre = T * (1 - clamp(con_o.w * exp(power), 0.0f, 0.99f));
			if (test_T_pre < 0.0001f)
			{
				done = true;
				continue;
			}
			...
			if(add_tex){
				...
				float weight_1 = scale_x * scale_y / weight_total;
				float weight_2 = scale_y * scale_z / weight_total;
				float weight_3 = scale_z * scale_x / weight_total;
			...
			float gain = 1.0f + this_opacity_residue;              // (1 + r)
			float alpha = fminf(0.99f, fmaxf(0.0f, con_o.w * exp(power) * gain));
