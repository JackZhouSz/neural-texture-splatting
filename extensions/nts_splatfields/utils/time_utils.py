...
class SplatFields(nn.Module):
    def __init__(self, radius=None, n_frames=0, **kwargs):
        ...
        self.cp_decomposition = kwargs.get('cp_decomposition', False)
        self.opacity_dim =  3
        self.color_dim = 9
        uv_res = 4
        self.uv_res = uv_res
        self.uv_full_dim = self.uv_res * 2 if self.cp_decomposition else self.uv_res * self.uv_res
         
        self.tex_encoder = eval(self.encoder_type)(encoder_args)
        self.mlp_refine_feat_tex = torch.nn.Sequential(
            torch.nn.Linear(self.feat_dim, self.feat_dim),
            torch.nn.ReLU(),
            torch.nn.Linear(self.feat_dim, self.feat_dim),
        )
        ...
        self.mlp_color_residue = SirenMLP(
            in_features=self.feat_dim + 3 + time_input_ch, 
            # out_features=uv_res*uv_res, 
            out_features=self.uv_full_dim * self.color_dim, 
            hidden_features=kwargs.get('rgb_w', 128), 
            num_hidden_layers=kwargs.get('rgb_d', 4), 
            out_activation='tanh'
        )

        if not self.cp_decomposition:
            nn.init.constant_(self.mlp_color_residue.net[-1].weight, 0.0)
            nn.init.constant_(self.mlp_color_residue.net[-1].bias, 0.0)
        else:
            nn.init.constant_(self.mlp_color_residue.net[-1].weight, 0.0)
            nn.init.constant_(self.mlp_color_residue.net[-1].bias, 0.01)

    
        # self.mlp_opacity_residue = GeneralMLP(
        #     in_features=3+self.feat_dim+time_input_ch, 
        #     # in_features=3+self.feat_dim, 
        #     # out_features=uv_res*uv_res*3, 
        #     out_features=self.uv_full_dim * self.opacity_dim,
        #     hidden_features=kwargs.get('rgb_w', 128), 
        #     num_hidden_layers=kwargs.get('rgb_d', 2), 
        #     skips=kwargs.get('rgb_skips', []),
        #     multires=kwargs.get('rgb_multires', 0), 
        #     out_activation='tanh',
        #     act='leaky_relu', 
        #     composition_rank=composition_rank, n_frames=n_frames
        # )
        # if not self.cp_decomposition:
        #     nn.init.constant_(self.mlp_opacity_residue.net[-1].weight, 0.0)
        #     nn.init.constant_(self.mlp_opacity_residue.net[-1].bias, 0.0)
        # else:
        #     nn.init.constant_(self.mlp_opacity_residue.net[-1].weight, 0.0)
        #     nn.init.constant_(self.mlp_opacity_residue.net[-1].bias, 0.01)
    ...
    def extract_features_tex(self, x, t):
        if t is not None:
            t_feat = self.embed_time_fn(t) if self.n_frames > 0 else None
        else:
            t_feat = None
        x_feat = self.tex_encoder(x[None]).squeeze(0) if self.feat_dim > 0 else None
        if x_feat is not None:
            x_feat = self.mlp_refine_feat_tex(x_feat)
    ...
    def forward(self, xyz_in, t, use_tex=True):
        to_ret = {}
        ...
        if use_tex:
            pts_feat = self.extract_features_tex(xyz_in.detach(), t)
            color_tex = self.mlp_color_residue(torch.cat([pts_feat, xyz_can.detach()], dim = -1))
            if self.cp_decomposition:
                color_base = color_tex.reshape(-1, self.uv_res + self.uv_res, self.color_dim)
                ...
                color_tex = color_vector_x * color_vector_y   # [B, uv_res, uv_res, self.tex_color_dim]
                color_tex = color_tex.reshape(means3D.shape[0], 4, 4, self.color_dim)

            else:
                color_tex = color_tex.reshape(means3D.shape[0], 4, 4, self.color_dim)
            to_ret.update({'color_tex':color_tex})

            # opacity_tex = self.mlp_opacity_residue(torch.cat([pts_feat, xyz_can], dim = -1))
            # opacity_tex = self.mlp_opacity_residue(xyz=xyz_can, xyz_feat=pts_feat, frame_id=frame_id)
            # if self.cp_decomposition:
            #     opacity_base = opacity_tex.reshape(-1, self.uv_res + self.uv_res, self.opacity_dim)
            ...
            #     opacity_tex = opacity.reshape(means3D.shape[0], 4, 4, self.opacity_dim)
            # else:
            #     opacity_tex = opacity_tex.reshape(means3D.shape[0], 4, 4, self.opacity_dim)
            # opacity_tex = opacity_tex * 0.1

            opacity_tex = torch.zeros((means3D.shape[0], 4, 4, self.opacity_dim), device=means3D.device, dtype=means3D.dtype)
            
            to_ret.update({'opacity_tex':opacity_tex})
