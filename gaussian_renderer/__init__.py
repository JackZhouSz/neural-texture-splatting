...
import torch
import math
from diff_gaussian_rasterization_tex import GaussianRasterizationSettings, GaussianRasterizer
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, kernel_size: float, scaling_modifier = 1.0, override_color = None, subpixel_offset=None):
    ...
    # filter_mask = None
    filter_mask = (opacity > 0.05).squeeze(-1)
    if pc.contract:
        filter_mask &= pc.get_xyz.abs().max(dim=-1)[0] < 15.0


    dir_pp = (pc.get_xyz - viewpoint_camera.camera_center.repeat(pc.get_features.shape[0], 1))
    dir_pp_normalized = dir_pp/dir_pp.norm(dim=1, keepdim=True)
    dir_pp_normalized = dir_pp_normalized.reshape(means3D.shape[0], 3).detach()

    opacity_residue = pc.get_opacity_residue(dir_pp_normalized, filter_mask)
    color_residue = pc.get_color_residue(dir_pp_normalized, filter_mask)
