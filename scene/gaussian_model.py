...
from scene.tex_net import TexNet, contract_mipnerf360, contract_unit
...
class GaussianModel:
    ...
    def __init__(self, model_params):
        ...
        self.use_uv = model_params.use_uv
        self.uv_res = model_params.uv_res
        self.contract = model_params.contract
        self.contract_bbox = model_params.contract_bbox
        self.view_dependent_color = model_params.view_dependent_color
        self.decompose_type = model_params.decomposition_type
        self.tex_delay_iters = model_params.tex_delay_iters

        self.tex_net = TexNet(self.uv_res, decompose_type=self.decompose_type, view_dependent_color=self.view_dependent_color).cuda()
    ...
    def restore_inference(self, model_args):
        (self.active_sh_degree, 
        self._xyz, 
        self._features_dc, 
        self._features_rest,
    ...
    def get_opacity_residue(self, view_dir, filter_mask):
        if self.use_uv and (self.iteration > self.tex_delay_iters or self.iteration == -1):
            ...
            if self.contract:
                query_xyz = contract_unit(query_xyz, self.contract_bbox[0], self.contract_bbox[1])
    ...
    def get_color_residue(self, view_dir, filter_mask):
        if self.use_uv and (self.iteration > self.tex_delay_iters or self.iteration == -1):
            ...
            if view_dir is not None and self.view_dependent_color:
                ...
                addition_feat = torch.cat([addition_feat, view_emb], dim=-1)
    ...
    def training_setup(self, training_args):
        ...
        if self.use_uv:
            l += [
                {'params': self.tex_net.parameters(), 'lr': training_args.tex_net_lr, "name": "tex_net"}
            ]
            self.delayed_lrs["tex_net"] = training_args.tex_net_lr
    ...
    def _update_params(self, idxs, ratio):
        ...
        def compute_relocation_cuda(opacity_old, scale_old, N):
            from diff_gaussian_rasterization_mcmc import compute_relocation
            # import math
        ...
        new_opacity, new_scaling = compute_relocation_cuda(
            ...
            # scale_old=torch.cat((self.get_scaling[idxs], torch.zeros_like(self.get_scaling[idxs][:, :1])), dim=-1),
            N=ratio[idxs, 0] + 1
        )
